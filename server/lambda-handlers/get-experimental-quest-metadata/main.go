...
func init() {
	svc = &service{db: database.Connect("experimental_quest_metadata")}
}
...
func handler(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	log.Printf("get-experimental-quest-metadata: fetching metadata for slug=%s", questSlug)
	quest, err := svc.GetQuestBySlug(questSlug)
	if err != nil {
		log.Printf("get-experimental-quest-metadata: error fetching quest: %v", err)
