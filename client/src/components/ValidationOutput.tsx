...
const ValidationOutput: FC<ValidationOutputProps> = ({ errors, suggestions, isVisible, onToggle, onClear }) => {
  ...
  if (!isVisible) {
    return (
      <div className="h-8 bg-gray-800 dark:bg-gray-900 border-t border-gray-600 flex items-center px-3">
        ...
        >
          ▶ Validation ({errors.length + suggestions.length})
        </button>
      </div>
    );
  }
    ...
    >
        ...
        <div className="flex items-center gap-3">
          <div className="flex gap-2 text-xs">
            ...
            {suggestions.length > 0 && (
              <span className="text-yellow-400">💡 {suggestions.length} suggestions</span>
            )}
          </div>
          <button
            onClick={onClear}
      ...
      <div className="flex-1 overflow-y-auto p-2 font-mono text-sm">
        ...
        {suggestions.map((suggestion, i) => (
          <div key={`suggestion-${i}`} className="text-yellow-400 mb-1">
            {suggestion}
          </div>
        ))}
        {errors.length === 0 && suggestions.length === 0 && (
          <div className="text-gray-500 italic">No validation issues</div>
        )}
      </div>
    </div>
  );
